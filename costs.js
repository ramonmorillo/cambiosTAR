...
(function () {
  ...
  function impactRows(records) { return records.map((r) => { const c = window.CambiosCosts.publicCostColumns(r, window.CambiosCostsState || {}); const label = impactLabel(c); return { fecha_cambio: r.fecha, paciente_pseudonimizado: r.patient_id, tar_anterior: r.tar_antiguo_normalizado || r.tar_antiguo, coste_anual_tar_anterior: c.coste_anual_tar_anterior_eur, tar_nuevo: r.tar_nuevo_normalizado || r.tar_nuevo, coste_anual_tar_nuevo: c.coste_anual_tar_nuevo_eur, diferencia_anual: c.coste_calculable === 'si' ? impactText(c.diferencia_anual_eur, true) : 'No calculable', diferencia_anual_eur: c.diferencia_anual_eur, resultado: label, impacto_economico: label, interpretacion: interpretation(label), motivo_cambio: r.motivo_normalizado }; }); }
  ...
  function notFoundExportRows(records) { const map = new Map(); records.filter((r) => r.impacto_coste?.coste_calculable !== 'si').forEach((r) => [['old', r.impacto_coste?.missing_old, r.fecha], ['new', r.impacto_coste?.missing_new, r.fecha]].forEach(([side, value, fecha]) => { if (!value) return; const normalized = window.CambiosCosts.normalize(value); const item = map.get(normalized) || { tar_registrado: value, valor_normalizado: normalized, aparece_como_tar_anterior: 'no', aparece_como_tar_nuevo: 'no', frecuencia: 0, primera_fecha: fecha || '', ultima_fecha: fecha || '', sugerencia: 'Añada este TAR al catálogo de costes o cree un alias equivalente.' }; item.frecuencia += 1; if (side === 'old') item.aparece_como_tar_anterior = 'si'; else item.aparece_como_tar_nuevo = 'si'; if (fecha && (!item.primera_fecha || fecha < item.primera_fecha)) item.primera_fecha = fecha; if (fecha && (!item.ultima_fecha || fecha > item.ultima_fecha)) item.ultima_fecha = fecha; map.set(normalized, item); })); return Array.from(map.values()).sort((a,b)=>b.frecuencia-a.frecuencia); }
